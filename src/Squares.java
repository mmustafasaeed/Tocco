...
public class Squares extends ShapePanel {
	...
	public Squares(int x, int y, int side, boolean state) {
		super();
		this.state=state;
		origin = new Rectangle(x, y, side, side);
	...
	public void reSize(int j, int deltaX, int deltaY) {
		...
		case 0:
			if (origin.width-deltaX > 10)
				origin.setBounds(origin.x+deltaX, origin.y+deltaX, origin.width-deltaX, origin.height-deltaX);
			else
				origin.setBounds(origin.x+origin.width-10, origin.y+origin.height-10, 10, 10);
			break;
		case 1:
			if (origin.width-deltaY > 10)
				origin.setBounds(origin.x, origin.y+deltaY, origin.width-deltaY, origin.height-deltaY);
			else
				origin.setBounds(origin.x, origin.y+origin.height-10, 10, 10);
			break;
		case 2:
			if (origin.width+deltaX > 10)
				origin.setBounds(origin.x, origin.y, origin.width+deltaX, origin.height+deltaX);
			else
				origin.setBounds(origin.x, origin.y, 10, 10);
			break;
		case 3:
			if (origin.width-deltaX > 10)
				origin.setBounds(origin.x+deltaX, origin.y, origin.width-deltaX, origin.height-deltaX);
			else
